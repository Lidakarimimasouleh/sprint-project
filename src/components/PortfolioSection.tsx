...
const FALLBACK: Project[] = [
  { image: "/Image 32.png", chips: ["Social Media", "Photography"], title: "Surfers Paradise" },
  { image: "/Image 33.png", chips: ["Social Media", "Photography"], title: "Agency 976" },
  { image: "/Image 34.png", chips: ["Social Media", "Photography"], title: "Cyberpunk Caffe" },
  { image: "/Image 35.png", chips: ["Social Media", "Photography"], title: "Minimal Playground" },
];

const QUERY = `*[_type == "portfolioProject"] | order(order asc) { image, tags, title }`;
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="relative w-full overflow-hidden aspect-[4/5]">
        ...
        <div className="absolute bottom-4 left-4 flex gap-3">
          {project.chips.map((chip) => (
            <span key={chip} className="rounded-[24px] bg-white/30 backdrop-blur-[20px] px-3 py-1.5 text-[12px] font-mono text-[#1f1f1f] leading-none whitespace-nowrap">
              {chip}
            </span>
          ))}
        </div>
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        <Image src="/Link Arrow.svg" alt="View project" width={24} height={24} className="shrink-0" />
      </div>
    </div>
  );
}
...
function CTABox() {
  return (
    <div className="relative flex flex-col gap-6 p-8">
      ...
      <div>
        <a href="#" className="inline-flex items-center bg-[#1f1f1f] text-white px-7 py-4 rounded-[16px] text-[16px] font-medium leading-none">
          Let&apos;s talk
        </a>
      </div>
    </div>
    ...
    <section className="w-full bg-white py-[80px] px-4 md:px-8">
      ...
      <div className="flex items-start justify-between mb-12">
        <div className="flex items-start gap-3">
          ...
          <span className="font-mono text-[14px] leading-[1.1] text-[#1f1f1f] mt-1 shrink-0">004</span>
        </div>
        <div className="font-mono text-[14px] uppercase leading-[1.1] shrink-0 text-[#1f1f1f]" style={{ writingMode: "vertical-rl", transform: "rotate(180deg)" }}>
          [ portfolio ]
        </div>
      </div>
